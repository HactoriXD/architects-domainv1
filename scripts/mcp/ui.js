    ...
    function renderMcpServers() {
        ...
        el.mcpServerList.innerHTML = servers.map(server => {
            ...
            const capabilityHtml = tools.length
                ? tools.map(tool => `<span class="mcp-tool">${escapeHtml(tool.name)}</span>`).join('')
                : `<span class="mcp-tool muted">${preset.requiresBridge ? 'Needs HTTP bridge' : 'No tools listed'}</span>`;
            return `
